...
');
  ...
  private function GetNewsEditorEdit(&$template)
  {
	...
	$this->_mysql->Query("update translable_element set ".$langID."='".str_replace("'", "\'", $_POST['bbcodeText'])."' where id=".$row['content']);

	$this->_mysql->Query("update translable_element set ".$langID."='".str_replace("'", "\'", $_POST['title'])."' where id=".$row['title']);
